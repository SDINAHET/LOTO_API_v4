...
//             log.info("LOGIN_SUCCESS email={}", loginRequest.getEmail());
...
//      * - génère un JWT stocké dans un cookie
//      * - bloque après 3 mauvais mots de passe (403)
//      */
//     @Operation(
...
//             // 🔐 3. Vérifier qu'il est admin
//             if (!user.isAdmin()) {
//                 return ResponseEntity.status(HttpStatus.FORBIDDEN)
//                         .body(Map.of("message", "Accès réservé aux administrateurs"));
//             }

//             // 🔐 4. Vérifier le mot de passe via Spring Security
//             Authentication authentication = authenticationManager.authenticate(
...
//             Cookie jwtCookie = new Cookie("jwtToken", jwt);
//             jwtCookie.setHttpOnly(false);   // true en prod si tu veux empêcher JS
...
//                     ));
//         } catch (Exception e) {
//             // ❌ Mauvais mot de passe ou autre erreur d'authentification
//             int newAttempts = loginAttempts.getOrDefault(email, 0) + 1;
//             loginAttempts.put(email, newAttempts);
